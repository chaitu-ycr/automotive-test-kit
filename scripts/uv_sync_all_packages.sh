...
# Sync all packages dependencies and create/update venv (Linux/macOS)
echo "Syncing all packages dependencies and creating/updating venv..."
...
echo "Running uv sync --all-packages --link-mode=copy using venv..."
if ! "$VENV_PY" -m uv sync --all-packages --link-mode=copy; then
  echo "Failed to sync dependencies with uv." >&2
  exit 1
fi
